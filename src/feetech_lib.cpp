...
FeetechServo::FeetechServo() : dirPin_(0)
{
}

bool FeetechServo::init(byte const& dirPin, HardwareSerial *serialPort,long const& baudRate)
{
...
bool FeetechServo::init(HardwareSerial *serialPort, long const& baudRate)
{
    // ToDo: Fix Serial Connection over USB
    return this->init(255, serialPort, baudRate);
}

bool FeetechServo::ping(byte const &servoId)
{
...
bool FeetechServo::setId(byte const &oldServoId, byte const &newServoId)
{
...
bool FeetechServo::setPositionOffset(byte const &servoId, int const &positionOffset)
{
...
int FeetechServo::getCurrentPosition(byte const &servoId)
{
    return readTwoBytesRegister(servoId, STSRegisters::CURRENT_POSITION);
}

int FeetechServo::getCurrentSpeed(byte const &servoId)
{
    return readTwoBytesRegister(servoId, STSRegisters::CURRENT_SPEED);
}

int FeetechServo::getCurrentTemperature(byte const &servoId)
{
    return readTwoBytesRegister(servoId, STSRegisters::CURRENT_TEMPERATURE);
}

float FeetechServo::getCurrentCurrent(byte const &servoId)
{
    int16_t current = readTwoBytesRegister(servoId, STSRegisters::CURRENT_CURRENT);
    return current * 0.0065;
}

bool FeetechServo::isMoving(byte const &servoId)
{
    byte const result = readRegister(servoId, STSRegisters::MOVING_STATUS);
    return result > 0;
}

bool FeetechServo::setTargetPosition(byte const &servoId, int const &position, int const &speed, bool const &asynchronous)
{
...
bool FeetechServo::setTargetVelocity(byte const &servoId, int const &velocity, bool const &asynchronous)
{
    return writeTwoBytesRegister(servoId, STSRegisters::RUNNING_SPEED, velocity, asynchronous);
}

bool FeetechServo::setTargetAcceleration(byte const &servoId, byte const &acceleration, bool const &asynchronous)
{
...
int FeetechServo::sendMessage(byte const &servoId,
...
bool FeetechServo::writeRegisters(byte const &servoId,
...
bool FeetechServo::writeRegister(byte const &servoId,
...
bool FeetechServo::writeTwoBytesRegister(byte const &servoId,
                                           byte const &registerId,
                                           int16_t const &value,
                                           bool const &asynchronous)
{
...
byte FeetechServo::readRegister(byte const &servoId, byte const &registerId)
{
    byte result = 0;
...
int16_t FeetechServo::readTwoBytesRegister(byte const &servoId, byte const &registerId)
{
...
int FeetechServo::readRegisters(byte const &servoId,
                                  byte const &startRegister,
...
int FeetechServo::receiveMessage(byte const& servoId,
                                   byte const& readLength,
                                   byte *outputBuffer)
{
    // ToDo fix pin??
...
void FeetechServo::convertIntToBytes(byte const& servoId, int const &value, byte result[2])
{
...
void FeetechServo::sendAndUpdateChecksum(byte convertedValue[], byte &checksum)
{
    port_->write(convertedValue, 2);
    checksum += convertedValue[0] + convertedValue[1];
}

void FeetechServo::setTargetPositions(byte const &numberOfServos, const byte servoIds[],
                                        const int positions[],
                                        const int speeds[])
{
    port_->write(0xFF);
...
void FeetechServo::determineServoType(byte const& servoId)
{
